...
public class S_Server extends S_ClassModule {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			server.DoStart(); 		// Call the start functions in all the loaded modules
							  		// Load a module by extending it from ClassModule 
							  		// And put the put the parent in the constructor
			/*int x= 120,y=1028;
			System.out.println(server.getWorldModule().getMap().getPlayerArea().get(x,y)+" "+server.getWorldModule().getMap().getMobArea().get(x,y)+" "+server.getWorldModule().getMap().getPvpArea().get(x,y));
			System.out.println(server.getWorldModule().getMap().getPvpArea().get(800,400));
			System.out.println(server.getWorldModule().getMap().getPvpArea().get(800,250));
			System.out.println(server.getWorldModule().getMap().getPvpArea().get(800,500));
			*/
			/*
			BufferedWriter filebuffer = new BufferedWriter(new FileWriter("coltest1.txt", false));
			for (int y = 0; y<1280;y++)
				for (int x = 0; x<1280;x++)
				{
					if (server.getWorldModule().getMapManager().getPlayerArea().get(x,y)==true)
						filebuffer.write("X");
					else
						filebuffer.write(" ");
			...
			System.out.println(server.getWorldModule().getMapManager().getPvpArea().get(702,481));
			filebuffer.close();
			*/
			while (true) {
	...
	/**
	 * @param databaseModule The databaseModule to set.
	 */
