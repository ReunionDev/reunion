...
public class S_SessionManager {
	...
	public void workSessions() {
		...
		Iterator<G_Player> player1Iter = S_Server.getInstance().getWorldModule().getPlayerManager().getPlayerListIterator();
		...
		while (player1Iter.hasNext()) {
			...
			if(statusUpdateTime.getTimeElapsedSeconds() >= 10){
				...
				//if(client.getState() == S_Enums.CS_INGAME){
					player1.updateStatus(0,player1.getCurrHp()+(int)(player1.getMaxHp()*0.1),player1.getMaxHp());
					player1.updateStatus(1,player1.getCurrMana()+(int)(player1.getMaxMana()*0.08),player1.getMaxMana());
					player1.updateStatus(2,player1.getCurrStm()+(int)(player1.getMaxStm()*0.08),player1.getMaxStm());
					player1.updateStatus(3,player1.getCurrElect()+(int)(player1.getMaxElect()*0.08),player1.getMaxElect());
				//}
			  }
			if(!statusUpdateTime.isRunning())
			statusUpdateTime.Start();
						
			Iterator<G_Player> player2Iter = world.getPlayerManager().getPlayerListIterator();
			while (player2Iter.hasNext()) {
				G_Player player2 = (G_Player) player2Iter.next();
				...
				double xcomp = Math.pow(player1.getPosX()
						- player2.getPosX(), 2);
				double ycomp = Math.pow(player1.getPosY()
						- player2.getPosY(), 2);
			...
			Iterator<G_Mob> mobIter = world.getMobManager().getMobListIterator();
			while (mobIter.hasNext()) {
				...
				else {player1.getSession().enterMob(mob,0);
					  if (distance <= 150 && mob.getAttackType() != -1)
						if(S_Server.getInstance().getWorldModule().getMapManager().getMobArea().get(((player1.getPosX()/10)-300),(player1.getPosY()/10)) == true)
								mob.moveToPlayer(player1,distance);
				}
			}
			
			Iterator<G_Npc> npcIter = world.getNpcManager().getNpcListIterator();
			while (npcIter.hasNext()) {
				...
				else player1.getSession().enterNpc(npc);
			}
