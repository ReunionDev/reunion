...
public class S_Session {
	...
	public void enterPlayer(G_Player player, int warping) {
		...
		S_Server.getInstance().getWorldModule().getWorldCommand().charIn(sessionOwner,
				player, warping);
	}
	...
	public void exitPlayer(G_Player player) {
		...
		S_Server.getInstance().getWorldModule().getWorldCommand().charOut(sessionOwner,
				player);
	}
	...
	public void enterNpc(G_Npc npc) {
		if (npcList.contains(npc))
			return;
		npcList.add(npc);
		S_Server.getInstance().getWorldModule().getWorldCommand().npcIn(sessionOwner,npc);
	}
	...
	public void exitNpc(G_Npc npc) {
		...
		while (npcList.contains(npc))
			npcList.remove(npc);
		S_Server.getInstance().getWorldModule().getWorldCommand().npcOut(sessionOwner,npc);
	}
	...
	public void enterMob(G_Mob mob, int spawning) {
		if (mobList.contains(mob))
			return;
		mobList.add(mob);
		S_Server.getInstance().getWorldModule().getWorldCommand().mobIn(sessionOwner,mob,spawning);
	}
	...
	public void exitMob(G_Mob mob) {
		...
		while (mobList.contains(mob))
			mobList.remove(mob);
		S_Server.getInstance().getWorldModule().getWorldCommand().mobOut(sessionOwner,
				mob);
	}
	...
	public void workSession(float radius) {
		...
		while (iter.hasNext()) {
			...
			if (player.getMap() != sessionOwner.getMap()) {
				...
				if (client != null) {
					...
					S_Server.getInstance().getNetworkModule().SendPacket(
							client.networkId, packetData);// send
				}
				removeList.add(player);
				continue;
			}
			double xcomp = Math.pow(sessionOwner.getPosX()
					- player.getPosX(), 2);

			double ycomp = Math.pow(sessionOwner.getPosY()
					- player.getPosY(), 2);
			 ...
			 * "+distance);
			 * S_Server.getInstance().getWorldModule().worldCommand.serverTell(sessionOwner,"Distance:
			 * "+(int)distance);
			 * S_Server.getInstance().getWorldModule().worldCommand.serverTell(sessionOwner,"Players
			 * in Your session:
			...
			if (distance > radius) {
				...
				removeList.add(player);
				S_Server.getInstance().getWorldModule().getWorldCommand().serverTell(
						sessionOwner, "Removing Player: "
								+ player.getName());
	...
	public G_Player getPlayer(int index) {
		return (G_Player) this.playerList.get(index);
	}
