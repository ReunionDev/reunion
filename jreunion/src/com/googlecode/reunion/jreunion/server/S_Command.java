...
public class S_Command {
	...
	void sendCharList(int networkId, int accountId) {
		S_Client client = S_Server.getInstance().getNetworkModule().getClient(networkId);

		S_Server.getInstance().getNetworkModule().SendPacket(networkId, S_DatabaseUtils
				.getInstance().getCharList(accountId));
	...
	void sendSuccess(int networkId) {
		S_Server.getInstance().getNetworkModule().SendPacket(networkId, "success\n");
		return;
	}

	void sendFail(int networkId) {
		S_Server.getInstance().getNetworkModule().SendPacket(networkId, "fail\n");
		return;
	}
	...
	{
		G_Player player = S_DatabaseUtils.getInstance().loadChar(slotNumber, accountId,networkId);
		...
		G_Equipment eq = S_DatabaseUtils.getInstance().getEquipment(player.getEntityId());
		...
		serverSay(player.getName() + " is logging in (ID: "+player.getEntityId()+")\n");

		String packetData = "skilllevel_all";
		Iterator<G_Skill> skillIter = player.getCharSkill().getSkillListIterator();
		while(skillIter.hasNext()){
			G_Skill skill = skillIter.next();
			packetData = packetData + " " +skill.getId() + " " + skill.getCurrLevel();
			// "skilllevel_all 1 25 2 25 17 0 18 25 19 0 31 0 37 0 38 0 39 0 40 25 41 0 60 0 61 0 71 0 75 0\n");
		}
		packetData = packetData +"\n";
		
		S_Server.getInstance().getNetworkModule().SendPacket(networkId,packetData);
		...
		packetData = "wearing " +eqHelmetId+ " " +eqHelmetType+ " " +eqHelmetGem+ " "
		+eqHelmetExtra+ " " +eqArmorId+ " " +eqArmorType+ " " +eqArmorGem+ " "
		+eqArmorExtra+ " " +eqPantsId+ " " +eqPantsType+ " " +eqPantsGem+ " " 
		+eqPantsExtra+ " " +eqShoulderMountId+ " " + eqShoulderMountType + " "
		+eqShoulderMountGem+ " " +eqShoulderMountExtra+ " " +eqBootsId+ " "
		+ eqBootsType + " " +eqBootsGem+ " " +eqBootsExtra+ " " +eqShieldId+ " "
		+ eqShieldType + " " +eqShieldGem+ " " +eqShieldExtra+ " " +eqNecklaceId+ " "
		+eqNecklaceType+ " " +eqNecklaceGem+ " " +eqNecklaceExtra+ " " +eqBraceletId+ " "
		+eqBraceletType+ " " +eqBraceletGem+ " " +eqBraceletExtra+ " " +eqRingId+ " "
		+eqRingType+ " " +eqRingGem+ " " +eqRingExtra+ " " +eqWeaponId+ " " 
		...
		S_Server.getInstance().getNetworkModule().SendPacket(networkId,S_PacketFactory.createPacket(S_PacketFactory.PT_OK));

		return;
	}
	public void serverSay(String text) {
		Iterator<G_Player> iter = world.getPlayerManager().getPlayerListIterator();

		while (iter.hasNext()) {
			G_Player pl = iter.next();

			S_Client client = S_Server.getInstance().getNetworkModule().getClient(pl);
			if (client == null)
				continue;
	...
	public void charIn(G_Player player1, G_Player player2, int warping) {
		...
		S_Client client = S_Server.getInstance().getNetworkModule().getClient(player1);
		...
		G_Equipment eq = S_DatabaseUtils.getInstance().getEquipment(player2.getEntityId());
		...
		if (eq.getFirstHand()!=null) eqFirstHand = eq.getFirstHand().getType();
		if (eq.getSecondHand()!=null) eqSecondHand = eq.getSecondHand().getType();
		...
		if(warping == 0)
			packetData = "in ";
		else packetData = "appear ";
		
		packetData += "char " + player2.getEntityId() + " "
				+ player2.getName() + " " + player2.getRace() + " "
				+ player2.getSex() + " " + player2.getHairStyle()
				+ " " + player2.getPosX() + " " + player2.getPosY()
				+ " " + player2.getPosZ() + " "
				+ player2.getRotation() + " " + eqHelmet + " " + eqArmor
				+ " " + eqPants + " " + eqShoulderMount+ " " + eqBoots + " "
				+ eqSecondHand + " " +  eqFirstHand + " "+percentageHp+" "+combat+" 0 0 0 0 0 0\n";
		// S> in char [UniqueID] [Name] [Race] [Gender] [HairStyle] [XPos]
		// [YPos] [ZPos] [Rotation] [Helm] [Armor] [Pants] [ShoulderMount]
		// [Boots] [Shield] [Weapon] [Hp%] [CombatMode] 0 0 0 [Boosted] [PKMode] 0 [Guild]
		// [MemberType] 1
		S_Server.getInstance().getNetworkModule().SendPacket(client.networkId, packetData);
	...
	public void mobIn(G_Player player, G_Mob mob, int spawn) {
		...
		String packetData = "in npc " + mob.getEntityId() + " "
		+ mob.getType() + " " + mob.getPosX() + " "
		+ mob.getPosY() + " 0 "	+ mob.getRotation() + " "
		+ percentageHp +" "+ mob.getMutant() +" "
		+ mob.getUnknown1()+" "+ mob.getNeoProgmare() +" 0 "+ spawn +" "
		+ mob.getUnknown2()+"\n";
		// in npc [UniqueID] [type] [XPos] [YPos] [ZPos] [Rotation] [HP] [MutantType] 0 [NeoProgmare] 0 0
		S_Server.getInstance().getNetworkModule().SendPacket(client.networkId, packetData);
	}
	...
	public void npcIn(G_Player player, G_Npc npc) {
		...
		String packetData = "in npc " + npc.getEntityId() + " "
		+ npc.getType() + " " + npc.getPosX() + " "
		+ npc.getPosY() + " 0 "	+ npc.getRotation() + " 100 0 0 0 0 0 10\n";
	...
	public void dropItem(G_Player player, int itemtype, int posX, int posY,
		...
		com.googlecode.reunion.jreunion.game.G_EntityManager.getEntityManager().createEntity(item);
	...
	public void itemIn(G_Player owner, int uniqueid, int itemtype,int posX,
		...
		S_Client client = S_Server.getInstance().getNetworkModule().getClient(owner);
		int networkId = client.networkId;
	...
	public void normalAttack(G_Player player, int uniqueId) { 
		...
		G_LivingObject livingObject = S_Server.getInstance().getWorldModule().getMobManager().getMob(uniqueId);
		
		if(livingObject == null){
			livingObject = S_Server.getInstance().getWorldModule().getPlayerManager().getPlayer(uniqueId);
		...
		int percentageHp = (livingObject.getCurrHp()*100)/livingObject.getMaxHp();
		...
		if (player.getSession().getPlayerListSize() > 0){
			...
			while(playerIter.hasNext()){
				...
				if(livingObject instanceof G_Mob)
					packetData = "attack char "+player.getEntityId()+" npc "+uniqueId+
					" "+percentageHp+" 0 0 0 0\n";
				else
					packetData = "attack char "+player.getEntityId()+" char "+uniqueId+
					" "+percentageHp+" 0 0 0 0\n";
	...
	public void subAttackChar(G_Player player1, int uniqueId) {
		S_Client client = S_Server.getInstance().getNetworkModule().getClient(player1);
		//G_Player player2 = (G_Player)G_EntityManager.getEntityManager().getEnt(uniqueId);
		
		String packetData = "attack_vital char "+player1.getEntityId()+" 100 0 0\n";
		S_Server.getInstance().getNetworkModule().SendPacket(client.networkId,packetData);
		...
		if (player1.getSession().getPlayerListSize() > 0)
			for (int i = 0; i < player1.getSession().getPlayerListSize(); i++) {
				...
				if (client == null)
					return;
				packetData = "effect 40 char "+player1.getEntityId()+" char "+uniqueId+
					" 100 0 0\n";
				S_Server.getInstance().getNetworkModule().SendPacket(client.networkId,
						packetData);
			}
	...
	public void subAttackNpc(G_Player player, int uniqueId) {
		...
		G_Mob mob = S_Server.getInstance().getWorldModule().getMobManager().getMob(uniqueId);
		...
		while(slayerDmg<spWeapon.getSpecialWeaponMinDamage() ||
			...
			slayerDmg = (Math.random()*100) + spWeapon.getSpecialWeaponMinDamage();
				
		System.out.print("Skill Level: "+player.getCharSkill().getSkill(40).getCurrLevel()+"\n");
		
		// Max normal attack damage * memory of the slayer * % skill (40) increase +
		// slayer attack damage * % skill (40) increase * 1 (if no demolition and *
		// 1.8 if demolition hit occurs)
		
		double dmg = player.getBestAttack() * (spWeapon.getMemoryDmg()/100) *
		((player.getCharSkill().getSkill(40).getCurrLevel()*20)/100) +
		slayerDmg * ((player.getCharSkill().getSkill(40).getCurrLevel()*20)/100) * 1;
		...
		String packetData = "sav npc "+uniqueId+" "+percentageHp+" 1 0 "+item.getExtraStats()+"\n";
		S_Server.getInstance().getNetworkModule().SendPacket(client.networkId,packetData);
		...
		if (player.getSession().getPlayerListSize() > 0)
			for (int i = 0; i < player.getSession().getPlayerListSize(); i++) {
				...
				if (client == null)
					continue;
				packetData = "sav npc "+uniqueId+" "+percentageHp+" 1 0 "+item.getExtraStats()+"\n";
				S_Server.getInstance().getNetworkModule().SendPacket(client.networkId,
						packetData);
			}
	...
	public void playerWeapon(G_Player player, int uniqueId) {
		...
		//C> pulse [SystemTime], [WeaponType], [WeaponUniqueID], [WeaponSpeed], 120
	...
	public void GoToPos(G_Player player, int posX, int posY) {
		...
		String packetData = "goto "+posX+" "+posY+" 0 "+player.getRotation()+"\n";
		S_Server.getInstance().getNetworkModule()
	...
	public void GoToChar(G_Player player, String charName) {
		S_Client client = S_Server.getInstance().getNetworkModule().getClient(player);
		G_Player player2 = S_Server.getInstance().getWorldModule().getPlayerManager().getPlayer(charName);
		...
		String packetData = "goto "+player.getPosX()+" "+player.getPosY()+" "+player.getPosZ()+" "+player.getRotation()+"\n";
		S_Server.getInstance().getNetworkModule()
				.SendPacket(client.networkId, packetData);
	}
		
	/******		mob attacks player with normal attack		******/
	public void NpcAttackChar(G_Player player, G_Mob mob) { //mob attacks player
		S_Client client = S_Server.getInstance().getNetworkModule().getClient(player);
		String packetData="";
		...
		case 0: {packetData = "attack npc "+mob.getEntityId()+" char "+player.getEntityId()+" "+(int)percentageHp+" "+mob.getDmgType()+" 0 0 0\n"; break;}
		case 1: {packetData = "attack npc "+mob.getEntityId()+" char "+player.getEntityId()+" "+(int)percentageHp+" "+mob.getDmgType()+" 0 0 0\n"; break;}
		case 2: {packetData = "attack npc "+mob.getEntityId()+" char "+player.getEntityId()+" "+(int)percentageHp+" "+mob.getDmgType()+" 0 0 0\n"; break;}
		default: break;
		}
		S_Server.getInstance().getNetworkModule().SendPacket(client.networkId,packetData);
