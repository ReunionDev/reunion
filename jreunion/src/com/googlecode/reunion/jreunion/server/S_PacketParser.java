...
public class S_PacketParser {
	...
	public void Parse(S_Client client, char[] packet) {
		...
		if (client.getState() != S_Enums.CS_DISCONNECTED)
			for (int i = 0; i < messages.length; i++) {
				String[] message = ParseMessage(messages[i]);
				if (client.getState() != S_Enums.CS_DISCONNECTED)
	...
	private void HandleMessage(S_Client client, String message[]) {
		...
		System.out
				.println("Parsing " + message[0] + " command on Client("
						+ client.networkId + ") with state: "
						+ client.getState() + "");
		...
		case S_Enums.CS_CHAR_LIST:
		{
			...
			if (message[0].equals("char_exist")) {
				...
				com.createChar(client, Integer.parseInt(message[1]),
						message[2], Integer.parseInt(message[3]), Integer
								.parseInt(message[4]), Integer
								.parseInt(message[5]), Integer
								.parseInt(message[6]), Integer
								.parseInt(message[7]), Integer
								.parseInt(message[8]), Integer
								.parseInt(message[9]), Integer
								.parseInt(message[10]));
				com.sendSuccess(client.networkId);
		...
		case S_Enums.CS_CHAR_SELECTED: {
			if (message[0].equals("start_game")) {
				...
				server.getNetworkModule().SendPacket(client.networkId, "status 11 "
						+ pl.getTotalExp() + " 0\n");
				server.getNetworkModule().SendPacket(client.networkId, "status 12 "
						+ pl.getLvlUpExp() + " 0\n");
				server.getNetworkModule().SendPacket(client.networkId, "status 13 "
						+ pl.getStatusPoints() + " 0\n");
				server.getNetworkModule().SendPacket(client.networkId, "status 10 "
						+ pl.getLime() + " 0\n");
				server.getNetworkModule().SendPacket(client.networkId, "status 19 "
						+ pl.getPenaltyPoints() + " 0\n");
				server.getNetworkModule().SendPacket(client.networkId, "at "
						+ client.playerObject.getEntityId()  + " " + pl.getPosX() + " "
						+ pl.getPosY() + " " + pl.getPosZ()	+ " 0\n");
				...
				pl.updateStatus(0,pl.getCurrHp(),(pl.getStr()*1)+(pl.getCons()*2));
				pl.updateStatus(1,pl.getCurrMana(),(pl.getWis()*2)+(pl.getDex()*1));
				pl.updateStatus(2,pl.getCurrStm(),(pl.getStr()*2)+(pl.getCons()*1));
				pl.updateStatus(3,pl.getCurrElect(),(pl.getWis()*1)+(pl.getDex()*2));
				pl.updateStatus(13,-pl.getStatusPoints(),0);
				
				int statusPoints = (pl.getStr() + pl.getWis() + pl.getDex() + pl.getCons() + pl.getLead()) - 80;
		...
		case  S_Enums.CS_INGAME:
		{
			if (message[0].equals("walk")) {
				...
				client.playerObject.updateStatus(Integer.parseInt(message[1])+ 10, 1, 0);
			}
			else if (message[0].equals("pick")) {
				client.playerObject.pickupItem(Integer.parseInt(message[1]));
			}
			else if (message[0].equals("inven")) {
				client.playerObject.getInventory().moveItem(client.playerObject,Integer.parseInt(message[1]),Integer.parseInt(message[2]),Integer.parseInt(message[3]));
				...
				client.playerObject.dropItem(Integer.parseInt(message[1]));
					...
					com.normalAttack(client.playerObject,Integer.parseInt(message[2]));
			}
			else if (message[0].equals("subat")) {
				if (message[1].equals("char"))
					com.subAttackChar(client.playerObject,Integer.parseInt(message[2]));
				if (message[1].equals("npc"))
					com.subAttackNpc(client.playerObject,Integer.parseInt(message[2]));
			}
			else if (message[0].equals("pulse")) {
				if(Integer.parseInt(message[2].substring(0,message[2].length()-1)) == -1)
				{
					client.playerObject.setMinDmg(1);
					client.playerObject.setMaxDmg(2);
				}
				else
					com.playerWeapon(client.playerObject,Integer.parseInt(message[3].substring(0,message[3].length()-1)));
			}
				...
				if (message[2].equals("npc")){
					G_Mob mob = S_Server.getInstance().getWorldModule().getMobManager().getMob(Integer.parseInt(message[3]));
					client.playerObject.useSkill(mob,Integer.parseInt(message[1]));
				}
				else if (message[2].equals("char")){
					G_Player player = S_Server.getInstance().getWorldModule().getPlayerManager().getPlayer(Integer.parseInt(message[3]));
					client.playerObject.useSkill(player,Integer.parseInt(message[1]));
				}
				...
				client.playerObject.getQuickSlot().quickSlot(client.playerObject,Integer.parseInt(message[1]));
			}
			else if (message[0].equals("go_world")) {
				com.GoWorld(client.playerObject,Integer.parseInt(message[1]),Integer.parseInt(message[2]));
			}
			else if (message[0].equals("use_quick")) {
				client.playerObject.getQuickSlot().useQuickSlot(client.playerObject,Integer.parseInt(message[1]));
			}
			else if (message[0].equals("move_to_quick")) {
				client.playerObject.getQuickSlot().MoveToQuick(client.playerObject,Integer.parseInt(message[1]),Integer.parseInt(message[2]),Integer.parseInt(message[3]));
			}
			else if (message[0].equals("quest")) {
					if (message[1].equals("cancel"))
						client.playerObject.getQuest().cancelQuest(client.playerObject);
					else{
						if (client.playerObject.getQuest()==null)
							client.playerObject.setQuest(new G_Quest(client.playerObject,Integer.parseInt(message[1])));
						
						else
							com.serverTell(client.playerObject,"Impossible to receive a quest");
					}
			}
			else if (message[0].equals("stash_open")) {
				G_Npc[] npc = S_Server.getInstance().getWorldModule().getNpcManager().getNpcList(139);
				...
				if (message.length == 5) //item insert/remove
					warehouse.stashClick(client.playerObject,Integer.parseInt(message[1]),
						Integer.parseInt(message[2]),Integer.parseInt(message[3]),
						Integer.parseInt(message[4]));
				if (message.length == 4) //lime deposite/out
					warehouse.stashClick(client.playerObject,Integer.parseInt(message[1]),
						Integer.parseInt(message[2]),Integer.parseInt(message[3]),0);
			}
			else if (message[0].equals("shop")) {
				G_Merchant npc = (G_Merchant)S_Server.getInstance().getWorldModule().getNpcManager().getNpc(Integer.parseInt(message[1]));
				...
				npc.buyItem(client.playerObject,Integer.parseInt(message[1]),
						Integer.parseInt(message[4]),Integer.parseInt(message[5]),1);
			}
				...
				G_Merchant npc = (G_Merchant)S_Server.getInstance().getWorldModule().getNpcManager().getNpc(Integer.parseInt(message[1]));
				npc.buyItem(client.playerObject,Integer.parseInt(message[1]),
						Integer.parseInt(message[2]),Integer.parseInt(message[3]),10);
			}
			else if (message[0].equals("chip_exchange")) {
				G_Npc[] npc;
				
				if(Integer.parseInt(message[1]) == 0){
					npc = S_Server.getInstance().getWorldModule().getNpcManager().getNpcList(166);
					G_Trader trader = (G_Trader)npc[0];
					trader.chipExchange(client.playerObject,Integer.parseInt(message[1]),
							Integer.parseInt(message[2]),0);
				}
				else{
					npc = S_Server.getInstance().getWorldModule().getNpcManager().getNpcList(167);
					G_Trader trader = (G_Trader)npc[0];
					trader.chipExchange(client.playerObject,Integer.parseInt(message[1]),
							Integer.parseInt(message[2]),Integer.parseInt(message[3]));
				}
				...
				G_Npc[] npc = S_Server.getInstance().getWorldModule().getNpcManager().getNpcList(117);
				G_Trader trader = (G_Trader)npc[0];
				
				if(message.length == 1)
					trader.exchangeArmor(client.playerObject,0);
				else
					trader.exchangeArmor(client.playerObject,Integer.parseInt(message[1]));
			}
