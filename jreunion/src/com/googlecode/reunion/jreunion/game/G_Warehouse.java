...
public class G_Warehouse extends G_Npc {
	...
	public void openStash(G_Player player) {
		...
		while(stashIter.hasNext()){
			G_StashItem stashItem = (G_StashItem)stashIter.next();
			
			String packetData = "stash "
				+stashItem.getPos()+ " "
		...
		S_Server.getInstance().getNetworkModule().SendPacket(client.networkId,"stash_end");
	}
	
	/******		Add/Remove items from stash		******/
	public void stashClick(G_Player player, int pos, int type, int gems, int special) {
		...
		if(pos == 12){
			...
			stashItem.getItem().setGemNumber(stashItem.getItem().getGemNumber()+(gems/100));
		  ...
		  if(player.getInventory().getItemSelected() == null){
			stashItem = stash.getItem(pos);
			G_Item item = S_ItemFactory.loadItem(stashItem.getItem().getEntityId());
			S_DatabaseUtils.getInstance().loadItemInfo(item);
			stash.removeItem(stashItem);
			player.getInventory().setItemSelected(new G_InventoryItem(item,0,0,0));
			...
			if(stash.checkPosEmpty(pos)){
				...
				if(stashItem.getItem().getType() == type &&
				    stashItem.getItem().getGemNumber() == player.getInventory().getItemSelected().getItem().getGemNumber() &&
				     stashItem.getItem().getExtraStats() == special){
						G_StashItem newStashItem = new G_StashItem(pos,player.getInventory().getItemSelected().getItem());
