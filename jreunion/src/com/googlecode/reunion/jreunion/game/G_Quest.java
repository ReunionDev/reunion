...
public class G_Quest {
	...
	public void getQuest(G_Player player, int slot){
		...
		updateMissionReceiver(player,slot,qsItem.getItem().getExtraStats()-1);
	}
	...
	public void changeQuestPT(G_Player player, int remainPoints, int obtainedPoints){
		S_Client client = S_Server.getInstance().getNetworkModule().getClient(player);
	...
	/****** 	Quest Points Reached Zero		********/
	/*public void QuestSecondFase(G_Player player){
		S_Client client = S_Server.getInstance().getNetworkModule().getClient(player);
	...
	public void updateMissionReceiver(G_Player player, int slot, int missionsRemaining){
		S_Client client = S_Server.getInstance().getNetworkModule().getClient(player);
	...
	public void questEff(G_Player player){
		...
		String packetData = "qt eff "+player.getPosX()+" "+player.getPosY()+" "+player.getEntityId()+"\n";
		S_Server.getInstance().getNetworkModule().SendPacket(client.networkId,packetData);
	}
	/****** 	Quest Kill		********/
